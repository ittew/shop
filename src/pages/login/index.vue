<template>
  <div class="page">
    <div class="login">
      ...
      <div class="contain">
        <div class="input bor-bot inputPh">
          <input class="input1 pdl" placeholder-class="phcolor" type="number" placeholder="请输入11位移动手机号码" maxlength="13" focus="true" @input="bindGetNum" />
        </div>
        <div class="smsBox">
          <div class="inpl bor-bot">
            <input class="input2 pdl" placeholder-class="phcolor" type="number" placeholder="请输入短信验证码" maxlength="6" @input="bindGetCode" />
          </div>
          <button class="get-code" size="mini"  @click="getCode" :disabled="disabled" style="color: #00a4ff; background-color: #fff;font-size:30rpx;">点击获取</button>
        </div>
      </div>
      <!-- 短信登录 -->
      <div class="btnBox loginBtn">
        <button :class="[isLog ? '' : 'button2']" open-type="getUserInfo" size="default" hover-class="other-button-hover" @click="login" :disabled="loginDisabled">登录</button>
      </div>
      <!-- 微信一键登录 -->
    ...
    <div v-show="!hideLoading" class='load-plug' id='load-plug'>
      <div class='o-wrap'>
        <div  class='o-lay'>
